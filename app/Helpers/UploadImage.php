...
class UploadImage
{
     ...
     * @return mixed
     */
    public function upload($input,$type='gallery')
    {
        ...
        $allowed_filename = $this->createUniqueFilename( $filename ,$type,$input['id']);

        $filename_with_extension = $allowed_filename .'.' . $extension;

        $manager = new ImageManager();

        if(!\File::isDirectory(config('dropzoner.upload-path') .$type.'/'.$input['id']))
            \File::makeDirectory(config('dropzoner.upload-path') .$type.'/'.$input['id'],0777);
        $image = $manager->make( $photo )->save(config('dropzoner.upload-path') .$type.'/'.$input['id'].'/'. $filename_with_extension );
     ...
     * @return mixed
     */
    public function delete($server_filename,$type='gallery',$id)
    {
        $upload_path = config('dropzoner.upload-path').$type.'/'.$id.'/';
     ...
     * @return string
     */
    private function createUniqueFilename( $filename,$type,$id )
    {
        $full_size_dir = config('dropzoner.upload-path').$type.'/'.$id.'/';
