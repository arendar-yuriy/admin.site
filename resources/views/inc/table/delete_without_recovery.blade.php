@if(Auth::user()->can([$controller.'-add-delete']))
    <td class="text-center">
        <a href="#" class="label label-warning label-icon remove-button-{{ $aRow['id'] }}"><i class="icon-bin"></i></a>
    </td>
    <script>
        $(document).ready(function(){
            ...
            $('.remove-button-{{ $aRow['id'] }}').on('click',function(e){
                ...
                bootbox.confirm("{{ trans('basket.basket_remove_confirm') }}", function(result) {
                    if(result){
                        $.ajax({
                            type: "GET",
                            url: url,
