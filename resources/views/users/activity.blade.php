                    ...
                    @foreach($logs as $item)
                        ...
                        <div class="timeline-row">
                            <div class="timeline-icon">
                                {!! MediaImage::getImage($content->image,40,40,['crop'=>$content->is_crop]) !!}
                            </div>
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                @foreach($item->items as $value)
                                                    <tr>
                                                        <td>{{ trans('activity.'.$value->action) }}</td>
                                                        <td>{{ $value->revisionable_id }}</td>
                                                        <td>{{ date('d-m-Y H:i:s',$value->at) }}</td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
